...
import { Lock, User, Sparkles, MessageCircle, X, Send, ShoppingCart, PhoneCall, MapPin, Mail, CreditCard, Banknote, Wallet, Trash2, Check } from 'lucide-react';
...
const ChatWidget = () => {
  ...
  const sendMessage = async () => {
    ...
    setIsLoading(true);// bắt đầu suy nghĩ 
    try {
      // TẠM THÊM 2 GIÂY DELAY ĐỂ BẠN CÓ THỂ NHÌN RÕ HIỆU ỨNG "ĐANG SUY NGHĨ" BẰNG MẮT THƯỜNG
      ...
      {isOpen && (
        <div className="chat-widget">
          ...
          <div className="chat-body" id="chat-body">
            ...
            {isLoading && (
              <div className="chat-message bot thinking-indicator" style={{ fontStyle: 'italic', opacity: 0.8 }}>
...
function App() {
  ...
  const [paymentDetails, setPaymentDetails] = useState({ method: "", orderId: "", items: [], total: 0 });
  ...
  const handleCheckout = (method) => {
    const orderId = `#MBP-2026-${Math.floor(10000 + Math.random() * 90000)}`;
    setPaymentDetails({ method, orderId, items: [...cart], total: cartTotal });
    ...
    <div className="App">
      ...
      {!currentUser ? (
        ...
        <div className="home-container">
          ...
          {isPaymentSuccess && (
            <div className="modal-overlay" style={{ backdropFilter: 'blur(15px)', zIndex: 3000 }} onClick={() => setIsPaymentSuccess(false)}>
              ...
              <div className="glass-panel modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '700px', width: '95%', textAlign: 'center', padding: '0', background: '#1c1c24', border: '1px solid rgba(138, 43, 226, 0.4)', boxShadow: '0 0 50px rgba(138, 43, 226, 0.3)', borderRadius: '20px', position: 'relative', zIndex: 3001, overflow: 'hidden' }}>
                
                {/* Header Section */}
                <div style={{ background: 'linear-gradient(180deg, rgba(138,43,226,0.15) 0%, rgba(28,28,36,0) 100%)', padding: '40px 30px 20px', borderBottom: '1px dashed rgba(255,255,255,0.1)' }}>
                  <div style={{ marginBottom: '15px' }}>
                    <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'rgba(0, 255, 255, 0.05)', display: 'flex', justifyContent: 'center', alignItems: 'center', margin: '0 auto', boxShadow: 'inset 0 0 20px rgba(0, 255, 255, 0.2), 0 0 30px rgba(0, 255, 255, 0.1)' }}>
                      <Check size={45} color="var(--accent-glow-secondary)" strokeWidth={3} style={{ filter: 'drop-shadow(0 0 8px rgba(0, 255, 255, 0.8))' }} />
                    </div>
                  </div>
                  <h2 style={{ fontSize: '2.2rem', marginBottom: '10px', color: 'var(--accent-glow-secondary)', fontWeight: 'bold', textShadow: '0 0 15px rgba(0,255,255,0.4)', letterSpacing: '1px' }}>ĐẶT HÀNG THÀNH CÔNG!</h2>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '1rem', margin: '0' }}>Mã giao dịch: <span style={{ color: '#fff', letterSpacing: '1px', fontFamily: 'monospace' }}>{paymentDetails.orderId}</span></p>
                </div>
                ...
                <div style={{ padding: '30px' }}>
                  ...
                  {/* Order Meta Grid */}
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px', background: 'rgba(0,0,0,0.3)', padding: '20px', borderRadius: '15px', border: '1px solid rgba(255,255,255,0.05)', marginBottom: '30px', textAlign: 'left' }}>
                    <div>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '1px' }}>Người Thu Nhận</p>
                      <p style={{ color: '#fff', fontSize: '1.2rem', fontWeight: 'bold', margin: '0' }}>{currentUser}</p>
                    </div>
                    <div>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '1px' }}>Giao Dịch Qua</p>
                      <p style={{ color: '#fff', fontSize: '1.1rem', margin: '0' }}>{paymentDetails.method}</p>
                    </div>
                    <div>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '1px' }}>Tổng Số Vật Phẩm</p>
                      <p style={{ color: '#fff', fontSize: '1.2rem', fontWeight: 'bold', margin: '0' }}>{paymentDetails.items.length} món</p>
                    </div>
                    <div>
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: '5px', textTransform: 'uppercase', letterSpacing: '1px' }}>Tổng Tiền</p>
                      <p style={{ color: 'var(--accent-glow-secondary)', fontSize: '1.4rem', fontWeight: 'bold', margin: '0', textShadow: '0 0 10px rgba(0,255,255,0.3)' }}>{paymentDetails.total.toLocaleString()} đ</p>
                    </div>
                  </div>

                  {/* Items List */}
                  <div style={{ textAlign: 'left' }}>
                    <h3 style={{ color: 'var(--text-primary)', fontSize: '1.1rem', marginBottom: '15px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '10px' }}>Chi tiết sản phẩm</h3>
                    <div style={{ maxHeight: '250px', overflowY: 'auto', paddingRight: '10px' }} className="cart-list">
                      {paymentDetails.items.map((item, idx) => (
                        <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: idx !== paymentDetails.items.length - 1 ? '15px' : '0', paddingBottom: idx !== paymentDetails.items.length - 1 ? '15px' : '0', borderBottom: idx !== paymentDetails.items.length - 1 ? '1px dashed rgba(255,255,255,0.05)' : 'none' }}>
                          <img src={item.image || "https://dummyimage.com/60x80/333/fff?text=?"} alt={item.name} style={{ width: '60px', height: '80px', objectFit: 'cover', borderRadius: '8px', border: '1px solid rgba(138,43,226,0.3)' }} />
                          <div style={{ flex: 1 }}>
                            <h4 style={{ color: '#fff', fontSize: '1.1rem', margin: '0 0 8px 0' }}>{item.name}</h4>
                            <span style={{ fontSize: '0.75rem', padding: '3px 8px', background: 'rgba(255,255,255,0.1)', borderRadius: '10px', color: 'var(--text-secondary)' }}>{item.category}</span>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <p style={{ margin: 0, fontWeight: 'bold', color: 'var(--accent-glow)', fontSize: '1.1rem' }}>{item.price.toLocaleString()} đ</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <button className="btn-login" style={{ width: '100%', padding: '16px', fontSize: '1.2rem', marginTop: '30px', margin: '30px auto 0 auto', display: 'block' }} onClick={() => setIsPaymentSuccess(false)}>
                    Khám Phá Tiếp
                  </button>
